...
@app.route('/')
@app.route('/music')
@app.route('/music/<int:genre_id>/menu')
def genreArtists(genre_id=2):
    genre = session.query(Genre).filter_by(id=genre_id).one()
    items = session.query(Artist).filter_by(genre_id=genre_id)
    return render_template(
        'menu.html', genre=genre, items=items, genre_id=genre_id)


@app.route('/music/<int:genre_id>/new', methods=['GET', 'POST'])
...
@app.route('/music/<int:genre_id>/<int:artist_id>/edit',
           methods=['GET', 'POST'])
def editArtistItem(genre_id, artist_id):
    if 'username' not in login_session:
        return redirect('/login')
...
@app.route('/music/<int:genre_id>/<int:artist_id>/delete',
           methods=['GET', 'POST'])
def deleteArtistItem(genre_id, artist_id):
    if 'username' not in login_session:
        return redirect('/login')
...
if __name__ == '__main__':
    app.secret_key = 'Y06Mw9a4UsHrVt8VBkCsdoDw'
